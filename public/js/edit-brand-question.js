...
function addOptionBlock(text = "", imageUrl = "", description = "", link = "") {
  ...
  block.innerHTML = `
    <div class="quiz-option-inner">
      ...
      <input type="text" name="optionTexts[]" value="${text}" placeholder="Your Brand´s Name" required />
      <input type="text" name="optionDescriptions[]" value="${description}" placeholder="Enter Description" />
      <input type="url" name="optionLinks[]" value="${link}" placeholder="Link to Your Brand" />
      ...
      <img class="quiz-selected-logo-preview" style="display:none; max-width:150px; margin-top:6px;" />
        <div class="quiz-option-logos"></div>
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    optionBlocks.forEach((block) => {
      ...
      if (activeLogo) {
        try {
          ...
          selectedLogoSrc = url.pathname; // ✅ only "/brand-logos/LinkedIn.png"
        } catch {
    ...
    try {
      const response = await fetch("/tvstation/brand/brand-question/update", {
        method: "POST",
        body: formData,
      });
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  loader.style.display = "flex";
  try {
    await fetchBrandLogos(); // Load logos once
