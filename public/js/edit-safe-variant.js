...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const showSafeIdProfileCheckbox = document.querySelector(
      'input[name="showSafeIdProfile"]'
    );
    formData.append(
      "showSafeIdProfile",
      showSafeIdProfileCheckbox?.checked ? "true" : "false"
    );
    ...
    try {
      const response = await fetch("/safe-id/safe-variant/update/details", {
        method: "POST",
        body: formData,
      });
...
function setupMediaProfileToggles() {
  const toggle = document.querySelector('input[name="showSafeIdProfile"]');
