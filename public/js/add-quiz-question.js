...
function setupMediaProfileTabs(block) {
  const container = block.querySelector(".quiz-media-profile-tabs");
  ...
  logoParentContainer.style.display = "none"; // Hide by default

  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const type = btn.dataset.type;
      const existingHidden = container.querySelector(
        `input[name="logoMediaProfile[]"][value="${type}"]`
      );
      ...
      if (btn.classList.contains("active")) {
        ...
        if (selectedCount < maxSelection) {
          ...
          hidden.type = "hidden";
          hidden.name = "logoMediaProfile[]";
          hidden.value = type;
          container.appendChild(hidden);
        } else {
          showToast(
            `You can select up to ${maxSelection} profiles only.`,
            "error"
          );
        }
      }
