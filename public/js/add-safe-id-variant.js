...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    // ⭐ ADD THIS ⭐
    const safeId = document.getElementById("safeId")?.value || "";
    ...
    const questionMessage = block
      .querySelector('input[name="questionMessage"]')
      .value.trim();

    if (questionImage) formData.append("questionImage", questionImage);
    if (questionLogo) formData.append("questionLogo", questionLogo);
    if (questionMessage) formData.append("questionMessage", questionMessage);

    formData.append(
      "generalPhoneNumber",
      block.querySelector('input[name="generalPhoneNumber"]').value.trim()
    );
    formData.append(
      "emergencyPhoneNumber",
      block.querySelector('input[name="emergencyPhoneNumber"]').value.trim()
    );
    formData.append(
      "otherPhoneNumber",
      block.querySelector('input[name="otherPhoneNumber"]').value.trim()
    );
    formData.append(
      "email",
      block.querySelector('input[name="email"]').value.trim()
    );
    ...
    try {
      const res = await fetch("/safe-id/safe-variant/create", {
        method: "POST",
        body: formData,
      });
