...
async function generateOptionBlock(block, index) {
  ...
  inputText.placeholder = "Name of Product";
  ...
  inputLink.type = "url";
  inputLink.placeholder = "Product Link";
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    options.forEach((opt, index) => {
      ...
      if (selectedLogo) {
        ...
        try {
          ...
          selectedLogoSrc = url.pathname; // e.g. "/product-logos/LinkedIn.png"
        } catch {
    ...
    try {
      const res = await fetch("/tvstation/product/product-question/create", {
        method: "POST",
        body: formData,
      });
      ...
      if (result.type === "success") {
        ...
        setTimeout(() => {
          window.location.href = `/tvstation/product/channels/${channelId}/session/${sessionId}/editquestion/${_id}`;
        }, 1000);
      } else {
...
async function createLogoSelectionContainer(previewImg) {
  ...
  const urls = await fetchProductLogos();
...
function setupUnifiedPreview(optDiv) {
  ...
  function clearAll() {
    ...
    clearBtn.style.display = "none";
    fileInput.value = "";
    fileLabelText.innerText = "Add Image";
    logoContainer
