...
router.get(
  "/channels/:channelId/session/:sessionId/portfolio",
  async (req, res) => {
    ...
    try {
      ...
      const total = await Portfolio.countDocuments({ channelId, sessionId });
      const hasMore = skip + portfolio.length < total;

      // ✅ Handle AJAX / JSON request
      if (req.xhr || req.headers.accept.includes("json")) {
        return res.json({ type: "success", data: portfolio, hasMore });
      }
      ...
      return res.render("dashboardnew", {
        ...
        activeSection: "portfolio",
        user: req.user,
        quizQuestions: portfolio, // keeping the same variable name used in frontend template
        hasMore,
        sessionId,
        channelId,
        session,
      ...
      console.error("Error fetching portfolios for session:", err);
      return res.render("dashboardnew", {
        channel: null,
...
router.get(
  "/channels/:channelId/session/:sessionId/addportfolioquestion",
  async (req, res) => {
    const { channelId, sessionId } = req.params;
    ...
    if (
      ...
      return res.render("portfolio/add-portfolio", {
        channel: null,
        session: null,
        error: "Invalid Channel or Session ID",
        user: req.user,
    ...
    try {
      ...
      const existingPortfolio = await Portfolio.findOne({ sessionId });
      if (existingPortfolio) {
        return res.render("portfolio/add-portfolio", {
          channel,
          session,
          error: "Only one portfolio is allowed per session.",
          user: req.user,
          sessionId,
        });
      }

      // ✅ Render add portfolio page
      return res.render("portfolio/add-portfolio", {
        channel,
        session,
        error: null,
      ...
      console.error(
        "Error in GET /channels/:channelId/session/:sessionId/addportfolio:",
        err
      );
      return res.render("portfolio/add-portfolio", {
        channel: null,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ✅ Create portfolio question
      const portfolioQuestion = new Portfolio({
        channelId,
        sessionId,
      ...
      await portfolioQuestion.save();

      res.status(201).json({
        message: "Portfolio question created successfully.",
        type: "success",
        data: portfolioQuestion,
      });
    } catch (err) {
      console.error("Error saving portfolio question:", err);
      cleanupUploadedFiles(req.files);
      res.status(500).json({
        message: "Failed to save portfolio question.",
        type: "error",
      });
    }
  }
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const question = await Portfolio.findOne({
        _id: questionId,
        channelId,
      }).lean();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ✅ Find Portfolio
      const portfolio = await Portfolio.findById(questionId);
      if (!portfolio) {
        cleanupUploadedFiles(req.files);
        return res
          .status(404)
          .json({ message: "Portfolio not found", type: "error" });
      }
      ...
      // ✅ Update main images
      portfolio.logo = handleImageUpdate("logo", req.files["logo"]?.[0]);
      portfolio.questionImage = handleImageUpdate(
        "questionImage",
        req.files["questionImage"]?.[0]
      );
      portfolio.questionLogo = handleImageUpdate(
        "questionLogo",
      ...
      if (clearedOptions) {
        ...
        fullyClearedOptionIds.forEach((id) => {
          const existingOpt = portfolio.options.find(
            (o) => o._id.toString() === id
          );
          if (existingOpt) deleteFileIfExists(existingOpt.image);
        });
        portfolio.options = portfolio.options.filter(
      ...
      const formattedOptions = parsedOptions.map((opt) => {
        ...
        let oldImage = null;
        if (opt._id) {
          const found = portfolio.options.find(
        ...
        // ✅ Case 3: New option uses a predefined logo
          ...
          opt.selectedLogoSrc &&
          opt.selectedLogoSrc.startsWith("/portfolio-logos/")
        ) {
      ...
      // ✅ Save updates
      portfolio.channelId = channelId;
      portfolio.sessionId = sessionId;
      portfolio.question = question.trim();
      portfolio.options = formattedOptions;
      portfolio.logoTitle = logoTitle?.trim() || null;
      portfolio.logoDescription = logoDescription?.trim() || null;
      portfolio.logoLink = logoLink?.trim() || null;
      portfolio.questionImageLink = questionImageLink?.trim() || null;
      portfolio.questionDescription = questionDescription?.trim() || null;
      portfolio.logoMediaProfile = logoProfiles;
      portfolio.showLogoSection =
...
// DELETE Portfolio event (same structure as Magic Screen)
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (!portfolio) {
      return res.status(404).json({
        message: "Portfolio event not found",
        type: "error",
      });
    }

    // Perform cascading deletion
    await cascadeDelete("portfolioQuestion", questionId);

    return res.status(200).json({
      message: "Portfolio event deleted successfully",
      type: "success",
    });
  } catch (err) {
...
router.get(
  "/channels/:channelId/session/:sessionId/portfolio-play",
  async (req, res) => {
    const { channelId, sessionId } = req.params;
    ...
    try {
      ...
      if (
        ...
        return res.render("portfolio/user-portfolio", {
          channel: null,
      ...
      if (!channel.isRunning) {
        return res.render("portfolio/user-portfolio", {
          channel: null,
          error: "Portfolio session is not currently running",
          user: req.user,
          currentQuestion: null,
          index: 0,
          total: 0,
      ...
      // ✅ Fetch Portfolio documents instead of MagicScreen
      const portfolios = await Portfolio.find({ sessionId })
        .sort({ createdAt: 1 })
        .skip(index)
        .limit(1)
        .lean();

      const total = await Portfolio.countDocuments({ sessionId });
      const currentQuestion = portfolios[0] || null;
...
router.post("/portfolio-response", async (req, res) => {
  const {
    questionId,
    channelId,
    sessionId,
  ...
  try {
    ...
    const question = await Portfolio.findById(questionId);
    if (!question) {
      return res.status(404).json({
        success: false,
        message: "Portfolio question not found",
      });
    }

    // ✅ Save Portfolio response
    const response = await PortfolioResponse.create({
      userId,
      questionId,
      channelId,
      sessionId,
    ...
    console.error("Error saving portfolio response:", err);
    return res.status(500).json({
      success: false,
      message: "Server error",
    });
...
router.get(
  "/channel/:channelId/session/:sessionId/portfolio-response-tracker",
  async (req, res) => {
    ...
    if (!channel || !channel.createdBy.equals(req.user._id)) {
      return res.render("dashboardnew", {
        portfolioResponses: [], // ✅ same key
        totalResponsesWithoutPagination: 0,
        currentPage: 1,
        totalPages: 0,
        activeSection: "portfolio-response-tracker",
        user: req.user,
        error: "Access denied",
        sessionId,
        channelId,
    ...
    const portfolioQrId = portfolioQuestion?.linkedQRCode || null;

    if (portfolioQrId) {
      const qrDoc = await QRCodeData.findById(portfolioQrId);
      if (qrDoc) {
    ...
    try {
      const result = await PortfolioResponse.aggregate([
        // Lookup Portfolio question
        {
          $lookup: {
            from: "portfolios",
      ...
      const portfolioResponses = result[0].data; // ✅ same key name
      ...
      res.render("dashboardnew", {
        portfolioResponses, // ✅ keep identical key for EJS reuse
      ...
      console.error("Error fetching portfolio responses:", error);
...
// ✅ API to return list of portfolio logos
router.get("/portfolio-logos", (req, res) => {
  const dir = path.join(__dirname, "..", "portfolio-logos");

  fs.readdir(dir, (err, files) => {
    if (err) {
      console.error("Error reading portfolio-logos folder:", err);
    ...
    // Build public URLs
    const urls = imageFiles.map((file) => `/portfolio-logos/${file}`);
