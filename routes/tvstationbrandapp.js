...
router.get(
  "/channels/:channelId/session/:sessionId/brand",
  async (req, res) => {
    ...
    try {
      ...
      const total = await Brand.countDocuments({ channelId, sessionId });
      const hasMore = skip + brand.length < total;

      // ✅ Handle AJAX / JSON request
      if (req.xhr || req.headers.accept.includes("json")) {
        return res.json({ type: "success", data: brand, hasMore });
      }
      ...
      return res.render("dashboardnew", {
        ...
        activeSection: "brand",
        user: req.user,
        quizQuestions: brand, // keeping the same variable name used in frontend template
        hasMore,
        sessionId,
        channelId,
        session,
      ...
      console.error("Error fetching brands for session:", err);
      return res.render("dashboardnew", {
        channel: null,
...
router.get(
  "/channels/:channelId/session/:sessionId/addbrandquestion",
  async (req, res) => {
    const { channelId, sessionId } = req.params;
    ...
    if (
      ...
      return res.render("brand/add-brand", {
        channel: null,
        session: null,
        error: "Invalid Channel or Session ID",
        user: req.user,
    ...
    try {
      ...
      const existingBrand = await Brand.findOne({ sessionId });
      if (existingBrand) {
        return res.render("brand/add-brand", {
          channel,
          session,
          error: "Only one brand is allowed per session.",
          user: req.user,
          sessionId,
        });
      }
      ...
      console.error(
        "Error in GET /channels/:channelId/session/:sessionId/addbrand:",
        err
      );
      return res.render("brand/add-brand", {
        channel: null,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ✅ Create brand question
      const brandQuestion = new Brand({
        channelId,
        sessionId,
      ...
      console.error("Error saving brand question:", err);
      cleanupUploadedFiles(req.files);
      res.status(500).json({
        message: "Failed to save brand question.",
        type: "error",
      });
    }
  }
...
router.get(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.render("brand/edit-brand", {
    ...
    try {
      ...
      const question = await Brand.findOne({
        _id: questionId,
        channelId,
      }).lean();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ✅ Find Brand
      const brand = await Brand.findById(questionId);
      if (!brand) {
        cleanupUploadedFiles(req.files);
        return res
          .status(404)
          .json({ message: "Brand not found", type: "error" });
      }
      ...
      const handleImageUpdate = (field, uploadedFile) => {
        if (cleared.includes(field)) {
          deleteFileIfExists(brand[field]);
          return null;
      ...
      // ✅ Update main images
      brand.logo = handleImageUpdate("logo", req.files["logo"]?.[0]);
      brand.questionImage = handleImageUpdate(
        "questionImage",
        req.files["questionImage"]?.[0]
      );
      brand.questionLogo = handleImageUpdate(
        "questionLogo",
      ...
      if (clearedOptions) {
        ...
        fullyClearedOptionIds.forEach((id) => {
          const existingOpt = brand.options.find(
            (o) => o._id.toString() === id
          );
          if (existingOpt) deleteFileIfExists(existingOpt.image);
        });
        brand.options = brand.options.filter(
      ...
      const formattedOptions = parsedOptions.map((opt) => {
        ...
        let oldImage = null;
        if (opt._id) {
          const found = brand.options.find(
        ...
        // ✅ Case 3: New option uses a predefined logo
          ...
          opt.selectedLogoSrc &&
          opt.selectedLogoSrc.startsWith("/brand-logos/")
        ) {
      ...
      // ✅ Save updates
      brand.channelId = channelId;
      brand.sessionId = sessionId;
      brand.question = question.trim();
      brand.options = formattedOptions;
      brand.logoTitle = logoTitle?.trim() || null;
      brand.logoDescription = logoDescription?.trim() || null;
      brand.logoLink = logoLink?.trim() || null;
      brand.questionImageLink = questionImageLink?.trim() || null;
      brand.questionDescription = questionDescription?.trim() || null;
      brand.logoMediaProfile = logoProfiles;
      brand.showLogoSection =
...
// DELETE Brand event (same structure as Magic Screen)
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (!brand) {
      return res.status(404).json({
        message: "Brand event not found",
        type: "error",
      });
    }

    // Perform cascading deletion
    await cascadeDelete("brandQuestion", questionId);

    return res.status(200).json({
      message: "Brand event deleted successfully",
      type: "success",
    });
  } catch (err) {
...
router.get(
  "/channels/:channelId/session/:sessionId/brand-play",
  async (req, res) => {
    const { channelId, sessionId } = req.params;
    ...
    try {
      ...
      if (
        ...
        return res.render("brand/user-brand", {
          channel: null,
      ...
      if (!channel.isRunning) {
        return res.render("brand/user-brand", {
          channel: null,
          error: "Brand session is not currently running",
          user: req.user,
          currentQuestion: null,
          index: 0,
          total: 0,
      ...
      // ✅ Fetch Brand documents instead of MagicScreen
      const brands = await Brand.find({ sessionId })
        .sort({ createdAt: 1 })
        .skip(index)
        .limit(1)
        .lean();

      const total = await Brand.countDocuments({ sessionId });
      const currentQuestion = brands[0] || null;
      ...
      console.error("Error loading brand question:", err);
      return res.render("brand/user-brand", {
        channel: null,
        error: "Server error. Please try again later.",
        user: req.user,
...
router.post("/brand-response", async (req, res) => {
  const {
    questionId,
    channelId,
    sessionId,
  ...
  try {
    // ✅ Verify Brand exists
    const question = await Brand.findById(questionId);
    if (!question) {
      return res.status(404).json({
        success: false,
        message: "Brand question not found",
      });
    }

    // ✅ Save Brand response
    const response = await BrandResponse.create({
      userId,
      questionId,
      channelId,
      sessionId,
    ...
    console.error("Error saving brand response:", err);
    return res.status(500).json({
      success: false,
      message: "Server error",
    });
...
router.get(
  "/channel/:channelId/session/:sessionId/brand-response-tracker",
  async (req, res) => {
    ...
    if (!channel || !channel.createdBy.equals(req.user._id)) {
      return res.render("dashboardnew", {
        brandResponses: [], // ✅ same key
        totalResponsesWithoutPagination: 0,
        currentPage: 1,
        totalPages: 0,
        activeSection: "brand-response-tracker",
        user: req.user,
        error: "Access denied",
        sessionId,
        channelId,
    ...
    let qrScanCount = 0;
    const brandQuestion = await Brand.findOne(
      { sessionId },
      "linkedQRCode"
    ).lean();

    const brandQrId = brandQuestion?.linkedQRCode || null;

    if (brandQrId) {
      const qrDoc = await QRCodeData.findById(brandQrId);
      if (qrDoc) {
    ...
    try {
      const result = await BrandResponse.aggregate([
        // Lookup Brand question
        {
          $lookup: {
            from: "brands",
      ...
      const brandResponses = result[0].data; // ✅ same key name
      ...
      res.render("dashboardnew", {
        brandResponses, // ✅ keep identical key for EJS reuse
      ...
      console.error("Error fetching brand responses:", error);
...
// ✅ API to return list of brand logos
router.get("/brand-logos", (req, res) => {
  const dir = path.join(__dirname, "..", "comment-logos");

  fs.readdir(dir, (err, files) => {
    if (err) {
      console.error("Error reading brand-logos folder:", err);
