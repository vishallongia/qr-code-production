...
router.get("/assign-qr-code/:qrCodeId?", async (req, res) => {
  try {
    ...
    if (!qrCodeId) {
      return res.status(400).send("QR Code ID is required");
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ message: "Qr Control Updated", user, type: "success" });
    } catch (error) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user.role === "demo-user") {
        return res.status(403).json({
          message:
            "Demo users cannot generate QR codes. Please contact the administrator.",
          type: "error",
        });
      }
